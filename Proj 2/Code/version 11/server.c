...
//discount kill command
void kill_user_manuel(int fd_to_server, int fd_to_user, int pid)
{
	//kill client
	write(fd_to_user, "__KILL_THIS_USER_SIGNAL__", MAX_MSG);
	usleep(1000);
	//close its pipes
	close(fd_to_user);
	close(fd_to_server);

	//kill the child that is polling in SERVER
	int status;
	kill(pid, SIGKILL);
...
int add_user(USER * user_list, int pid, char * user_id, int pipe_to_child, int pipe_to_parent)
{
	for (int i=0;i<MAX_USER;i++){
	if(strcmp(user_list[i].m_user_id, user_id) == 0)
		{
		kill_user_manuel(pipe_to_parent, pipe_to_child, pid);
		printf("Name Taken: %d", pid);
		
		//print message "name taken"
		return -1;
		}
	}
	
	printf ("Server, USER's PID: %d\n", pid);
...
{
	//kill client
	write(user_list[idx].m_fd_to_user, "__KILL_THIS_USER_SIGNAL__", MAX_MSG);
	usleep(1000);
	//close its pipes
	close(user_list[idx].m_fd_to_user);
	close(user_list[idx].m_fd_to_server);
...
int broadcast_msg(USER * user_list, char *buf, char *sender)
{
	...
	{
    if(user_list[i].m_status != SLOT_EMPTY) //if there is a user at that slot
		{
			//check if the user is the same as the sender
...
void send_p2p_msg(int idx, USER * user_list, char *buf)
{
	...
	int idx_to_user = find_user_index(user_list, user_name);

	//Extract the recipient name from string buf
	if (idx_to_user == -1)
	{
		printf ("ERROR: User Not Found\n");
		write(user_list[idx].m_fd_to_user, "ERROR: User Not Found", sizeof("ERROR: User Not Found"));
	}
	else
	{
		char text[MAX_MSG];
		...
		if (idx == idx_to_user)
		{
			strcpy (text, "You can't connect to yourself this way. Maybe try meditation or a walk in the woods instead?");
			write(user_list[idx].m_fd_to_user, text, MAX_MSG);
		}
		else
		{
			write(user_list[idx_to_user].m_fd_to_user, concat(user_list[idx].m_user_id, text), MAX_MSG);
		}
...
void exit_server(USER * user_list){
	...
	for (int i = 0; i < MAX_USER; i++)
	{
		...
		{
		//if there is a user there, kick the user (Kick also does cleanup)
    ...
    while(1)
    {
		...
		if (get_connection(user_id, pipe_child_writing_to_user, pipe_child_reading_from_user) == 0)
		{
			...
			else //parent
			{
				...
				//read(pipe_SERVER_reading_from_child[0], (char*)pid, MAX_MSG);

				if (add_user(user_list, pid, user_id, pipe_SERVER_writing_to_child[1], pipe_SERVER_reading_from_child[0]) == -1)
				{
					printf("You have reached the maximum number of users! ... stupid!\n");
					//NEED TO KILL THE OVERFLOW USERS
				}
				print_prompt("admin");
		...
		for(int i=0; i<MAX_USER; i++)
		{
		    if(user_list[i].m_status != SLOT_EMPTY)
			{
				if(read(user_list[i].m_fd_to_server, buf, MAX_MSG) != -1)
	        	{
