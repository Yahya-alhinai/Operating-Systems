...
* section: 22
* date: 11/10/2018
* name: Yahya Alhinai, Oguz Gurbuz
* id: 5181597, 5313002 */
#include <stdio.h>
#include <fcntl.h>
#include <errno.h>
...
int add_user(USER * user_list, pid_t pid, char * user_id, int pipe_to_child, int pipe_to_parent, int check);
...
void execCommand (int cmd, char *buf, USER * user_list, int i)
{
	...
	else if (cmd == KICK)
	{
		...
		{
			write(user_list[i].m_fd_to_user, "ERROR: This user does not have the permission to execute '\\kick' command", MAX_MSG);
		}
	}
	else if (cmd == P2P)
	{
		...
		{
			printf("Cannot send a private message from the SERVER\n");
		}
...
void send_p2p_msg(int idx, USER * user_list, char *buf)
{
	...
	{
		char * message = concat(user_list[idx].m_user_id, text);
		write(user_list[idx_target].m_fd_to_user, message, MAX_MSG);
		free(message);
	}
	memset(buf,'\0', MAX_MSG);
...
void broadcast_msg(USER * user_list, char *buf, char *sender)
{
	for(int i=0; i<MAX_USER; i++) //go through all users
	{
		if(user_list[i].m_status != SLOT_EMPTY && strcmp(user_list[i].m_user_id, sender) != 0) //if there is a user at that slot
		{
			...
			{
				char * message = concat("NOTICE", buf);
				write(user_list[i].m_fd_to_user, message, MAX_MSG);
				free(message);
			}
			else
			{
				char * message = concat(sender, buf);
				write(user_list[i].m_fd_to_user, message, MAX_MSG);
				free(message);
			}
		}
...
int add_user(USER * user_list, pid_t pid, char * user_id, int pipe_to_child, int pipe_to_parent, int check)
{
	...
	if (check == 2 || check == 1)//full OR duplicate name
	{
		return -1;
	}
	else
...
int main(int argc, char * argv[])
{
	...
	setup_connection("WE_DESERVE_100_ON_THIS_PROJECT"); // Specifies the connection point as argument.
	...
	while(1)
    {
		...
		{
			check = DupOflow(user_id, user_list);
			...
			else
			{
				...
				if (add_user(user_list, pid, user_id, pipe_SERVER_writing_to_child[1], pipe_SERVER_reading_from_child[0], check) == -1)
				{
					if (check == 2)//full
					{
					...
					else if (check == 1)//duplicate name
					{
		...
		for(int i=0; i<MAX_USER; i++)
		{
		    if(user_list[i].m_status != SLOT_EMPTY)
			{
    			...
    			{
					check = get_command_type(buf);
					if (check != P2P && check != KICK)
					{
						printf("%s: %s\n", user_list[i].m_user_id, buf);
						print_prompt("admin");
					}

					execCommand(check, buf, user_list, i);
					memset(buf,'\0', MAX_MSG);
    			}
			}
		}
