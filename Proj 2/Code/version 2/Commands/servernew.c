...
void kill_user(int idx, USER * user_list) {
	USER k_user = user_list[idx];
	int pid = k_user.m_pid;
	int kill (pid, SIGSTOP);
	waitpid(pid);
...
void cleanup_user(int idx, USER * user_list)
{
	...
	void *memset(user_list[i].m_user_id[MAX_USER_ID], 0, sizeof(user_list[i].m_user_id[MAX_USER_ID]));
	close(user_list[i].m_fd_to_user);
    close(user_list[i].m_fd_to_server);
	user_list[i].m_fd_to_user = -1;
    user_list[i].m_fd_to_server = -1;
	user_list[i].m_status = 1;
...
int broadcast_msg(USER * user_list, char *buf, char *sender)
{
	...
	for (i = 0; i < n; i++){
		...
		{
			if (user_list[i].m_user_id[MAX_USER_ID] != sender)
			{
...
{
	chat *user_name = extract_name(buf, user_list[idx].m_user_id);
	//not complete yet
