...
void send_p2p_msg(int idx, USER * user_list, char *buf)
{
	...
	strcpy (sender, user_list[idx].m_user_id);
	//Extract the recipient name from string buf
	extract_name(buf, user_name);
	idx = find_user_index(user_list, user_name);
	if (idx == -1){
		printf ("ERROR: User Not Found\n");
		write(user_list[senderID].m_fd_to_user, "ERROR: User Not Found", sizeof("ERROR: User Not Found"));
	}
	else{
		extract_text(buf, text);
		if (strcmp(sender, user_list[idx].m_user_id) == 0)
		{
			strcpy (text, "You are trying to send message to yourself, check again!");
		}
