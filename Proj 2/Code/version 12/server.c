...
void kill_user(pid_t pid, int m_fd_to_user, int m_fd_to_server)
{
	//kill client
	write(m_fd_to_user, "__KILL_THIS_USER_SIGNAL__", MAX_MSG);
	usleep(1000);
	//close its pipes
	close(m_fd_to_user);
	close(m_fd_to_server);
...
int add_user(USER * user_list, pid_t pid, char * user_id, int pipe_to_child, int pipe_to_parent)
{
	for (int i=0; i<MAX_USER; i++)
	{
		if(strcmp(user_list[i].m_user_id, user_id) == 0)
		{
			printf("ERROR: User name '%s' has aleady been taken\n", user_id);
			write(pipe_to_child, "ERROR: This user name has aleady been taken", MAX_MSG);
			return -1;
		}
	}
	...
	if (idx == -1)
	{
		...
		write(pipe_to_child, "ERROR: You have reached the maximum number of users!", MAX_MSG);
		return -1;
	}
	else
	{
...
{
	printf("USER '%s' with CHILD_PID '%d' is being KILLED\n", user_list[idx].m_user_id, user_list[idx].m_pid);

	kill_user(user_list[idx].m_pid, user_list[idx].m_fd_to_user, user_list[idx].m_fd_to_server);
...
void exit_server(USER * user_list)
{
	...
	//exit the parent (exit the program)
	exit(1);
}
/*
    ...
    while(1)
    {
		...
		if (get_connection(user_id, pipe_child_writing_to_user, pipe_child_reading_from_user) == 0)
		{
			...
			else //parent
			{
				...
				{
					kill_user(pid, pipe_SERVER_writing_to_child[1], pipe_SERVER_reading_from_child[0]);
