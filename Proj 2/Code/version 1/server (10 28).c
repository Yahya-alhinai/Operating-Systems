...
void runChild (int pipe_child_writing_to_user[2], int pipe_child_reading_from_user[2], int pipe_SERVER_reading_from_child[2], int pipe_SERVER_writing_to_child[2], char*buf)// move the child code to here, make main looks cleaner
{
  ...
  //make read non-blocking, so child can poll user/client
  if (fcntl(pipe_child_reading_from_user[0], F_SETFL, O_NONBLOCK) < 0)
      exit(2);
  ...
  // while polling check to see if user is dead (learn how to check if a process with pid given is still active)
    while(1)
    {
        ...
        if (read (pipe_child_reading_from_user[0], buf, MAX_MSG) != -1)
        {
            ...
            memset(buf,'\0', MAX_MSG); //clean buffer
        }
...
int main(int argc, char * argv[])
{
    ...
    // make stdin non-blocking so we can poll the terminal
    ...
    while(1)
    {
        ...
        //check if a user is trying to connect
        if (get_connection(user_id, pipe_SERVER_writing_to_child, pipe_SERVER_reading_from_child) == 0) //COME BAKE TO THOSE PIPES LATER!
        {
            // make pipe_SERVER_reading_from_child non-blocking
            ...
            printf ("%s\n", "Connected");
            //make two pipes to connect child with user
            ...
            {
                runChild(pipe_child_writing_to_user, pipe_child_reading_from_user, pipe_SERVER_reading_from_child, pipe_SERVER_writing_to_child, buf);
            }
            ...
            else //Parent (Server)
            {
                ...
                if (idx != -1)
                {
                    ...
                    add_user(idx, user_list, pid, user_id, pipe_to_child, pipe_to_parent)
                }
                */
                //-----------------------TEST--------------------------//
                ...
                while (1)
                {
                    ...
                    {
                      //child wrote something in the pipe, do something with it.
                    }
                }
                */
        ...
        if (read(0, buf, MAX_MSG) != -1)//read stdin
        {
          printf ("You typed: %s\n", buf);
          memset(buf,'\0', MAX_MSG); //clean buffer
          //there are data, do something with it.
        }
