...
void * dispatch(void *arg){
    ...
    int test;
    while (1) {
	fprintf (stderr,"%ld: Getting Connection...\n",pthread_self());
	...
	fprintf (stderr, "FD is: %d\n", fd);
        ...
        if (fd >= 0){
		test = get_request(fd, filename);
		fprintf (stderr, "test is: %d\n", test);
...
void * worker(void *arg){
	...
	fprintf (stderr, "Worker: getting request from queue");
