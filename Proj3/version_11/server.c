...
int cache_size, num_dispatcher, num_workers;
char *path;
/*
...
/* ************************************ Cache Code ********************************/
void logMe(int threadId, int reqNum, int fd, char *Request_string, int bytes_error, long time, char *cache_stat){
    fprintf(fptr, "[%d][%d][%d][%s][%d][%ldus][%s]\n", threadId, reqNum, fd, Request_string, bytes_error, time, cache_stat);
...
void addIntoCache(char *request, char *memory , int memory_size){
    ...
    logMe(0, 0, 0, request, 0, 0, "ERROR: Cache has reached its maximum size.");
}
...
void * dispatch(void *arg){
    ...
    while (1) {
        ...
        if (fd >= 0){   // Get request from the client
            ...
            while((nextIN + 1)%queueSize == nextOUT || abs(nextIN - nextOUT) == num_dispatcher) //queue full
                pthread_cond_wait(&CV_IN, &lock);

            fprintf(stderr, "|||||||||||||||||||||||||||||||||||||DISPATCH-START-[%ld]|||||||\n", pthread_self());
            fprintf(stderr, "DISPATCH: nextIN --> %d || nextOUT --> %d\n", nextIN, nextOUT);
            ...
            fprintf(stderr, "DISPATCH: numItem --> %d --> %d\n", numItem, (nextIN - nextOUT));
            fprintf(stderr, "DISPATCH: nextIN --> %d || nextOUT --> %d\n", nextIN, nextOUT);
...
void * worker(void *arg){
    int cacheIndex, reqNum, fd, bytes_error;
    ...
    while (1){
        ...
        fprintf(stderr, "WORKER: numItem --> %d --> %d\n", numItem, (nextIN - nextOUT));
        fprintf(stderr, "WORKER: nextIN --> %d || nextOUT --> %d\n", nextIN, nextOUT);
