...
void addIntoCache(char *request, char *memory , int memory_size){
    ...
    //free (cache[min].content);
    fprintf(stderr, "AFTER LOOP\n");
    //free (cache[min].request);
    cache[min].content = realloc(cache[min].content, memory_size);
    cache[min].request = realloc(cache[min].request, strlen(request));
...
void * dispatch(void *arg){
    ...
    while (1) {
        ...
        while(numItem == queueSize) //queue full OR
...
void * worker(void *arg){
    int index = *((int *) arg);
    thread_add(pthread_self(), index);
    int timeStart, timeEnd, cacheIndex, fd, reqNum, bytes_error, fileStat, size;
    char *request;
    char *content;
    ...
    while (1){
        ...
        if (fileStat != -1) //in disk/cache
        {
            ...
            if (cacheIndex == -1)
            fprintf (stderr, "return_result: %d\n", return_result (fd, getContentType(request), content, size));
            else
            fprintf (stderr, "return_result: %d\n", return_result (fd, getContentType(request), cache[cacheIndex].content, cache[cacheIndex].len));
        }
        else
        {
