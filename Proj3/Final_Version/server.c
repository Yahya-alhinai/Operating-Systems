...
void thread_init();
int thread_add(pthread_t ti, int index, int W_or_D);
void thread_kill();
...
int thread_add(pthread_t ti, int index, int W_or_D){
	...
	if (W_or_D == 0){
        if (threadArray[index].exists == 0){
            ...
            fprintf(fptr, "Dispatcher [%d] is created!\n", index);
            fprintf(stderr, "Dispatcher [%d] is created!\n", index);
            return 0;
        }
	}
    else{
        if (threadArray[index+num_dispatcher].exists == 0){
            threadArray[index + num_dispatcher].pthread_id = ti;
            threadArray[index + num_dispatcher].exists = 1;
            fprintf(fptr, "Worker [%d] is created!\n", index);
            fprintf(stderr, "Worker [%d] is created!\n", index);
            return 0;
        }
    }
    return -1;
...
void thread_kill(){
    for(int i = 0; i < 2*MAX_THREADS; i++){
        if(threadArray[i].exists == 1 && i < num_dispatcher){
            pthread_cancel(threadArray[i].pthread_id);
            fprintf(fptr, "Dispatcher [%d] is killed!\n", i);
            fprintf(stderr, "Dispatcher [%d] is killed!\n", i);
      	    threadArray[i].exists = 0;
	    }
        if(threadArray[i].exists == 1){
            pthread_cancel(threadArray[i].pthread_id);
            fprintf(fptr, "Worker [%d] is killed!\n", i - num_dispatcher);
...
int readFromDisk(char *request, char **content) {
    ...
    char dest[1024];
  	errno = 0;
    // Concat path and destination to get the absolute path for file
    strcpy(dest, path);
    strcat(dest, request);
    // Open file
  	fd = open(dest, O_RDONLY);
    // Check for any errors from opening the file
  	if (errno != 0){
        perror("ERROR");
        return -1;
    }
...
void * dispatch(void *arg){
    // Detach the thread upon initializing
    ...
    pthread_mutex_lock(&lock_3);
        thread_add(pthread_self(), index, 0);
...
void * worker(void *arg){
    ...
    //Needed because we don't want multiple thread have access to the log file at the sametime
    pthread_mutex_lock(&lock_3);
        thread_add(pthread_self(), index, 1);
    pthread_mutex_unlock(&lock_3);

    int timeStart, timeEnd, cacheIndex, fd, fileStat, size, reqNum = 0;
    char request[BUFF_SIZE];
    // Content from file will be stored in this variable
    char *content;
    ...
    while (1){
        ...
        if (cacheIndex == -1) //not in cache
        {
            ...
            if (size == -1)//not in disk
            {
                fileStat = -1;
            }
            else //found in disk --> put it and cache
            {
                addIntoCache(request, content, size);
                fileStat = 0;
            }
        }
        else
        ...
        if (fileStat != -1) //in disk/cache
        {
            ...
            // call up a error if the connection went wrong
            if (test != 0) fprintf(stderr, "Failed to send content for request [%s]\n", request);
...
int main(int argc, char **argv) {
    ...
    // Check that thread number spsecified are within range of 0 to MAX_THREADS
    if(num_dispatcher <= 0 || num_workers <= 0){
        printf("ERROR: can't have 0 or less threads\n");
        exit(-1);
    }
    ...
    // Check that Queue size is within range
    ...
    if(queueSize <= 0){
        printf("ERROR: Queue size must be larger than 0\n");
        exit(-1);
    }
    queueSize++;
    // Check that Cache size is within range
    ...
    if(cache_size <= 0){
        printf("ERROR: Cache size must be larger than 0\n");
        exit(-1);
    }
