    ...
    while (1){
        ...
        if (fileStat != -1) //in disk/cache
        {
            if (cacheIndex == -1)
            {
              ...
              test = return_result (fd, getContentType(request), content, size);
              if (test != 0)
                  exit(1);
            }
            else
            {
              ...
              test = return_result (fd, getContentType(cache[cacheIndex].request), cache[cacheIndex].content, cache[cacheIndex].len);
              if (test != 0)
                  exit(1);
            }
        }
