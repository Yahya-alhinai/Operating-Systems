...
void * dispatch(void *arg){
    ...
    pthread_mutex_lock(&lock_3);//Needed because we don't want multiple thread
                                //have access to the log file at the sametime
    ...
    while (1) {
        ...
        fprintf(stderr, "DISPATCH: FD = %d | request = %s | numItem = %d | save at index[%d]\n", boundedBuffer[nextIN].fd, boundedBuffer[nextIN].request, numItem, nextIN);
...
void * worker(void *arg){
    ...
    thread_add(pthread_self(), index);
    pthread_mutex_unlock(&lock_3);
    int timeStart, timeEnd, cacheIndex, fd, reqNum, fileStat, size;
    char request[BUFF_SIZE];
    char *content;
    ...
    while (1){
      ...
      /*  while(1){
        ...
        if (fileStat != -1) //in disk/cache
        {
            if (cacheIndex == -1)
            {
              logMe(index, reqNum, fd, request, "", timeEnd-timeStart, (cacheIndex == -1)? "MISS" : "HIT", size);
              fprintf (stderr, "return_result: %d\n", return_result (fd, getContentType(request), content, size));
            }
            else
            {
              logMe(index, reqNum, fd, request, "", timeEnd-timeStart, (cacheIndex == -1)? "MISS" : "HIT", cache[cacheIndex].len);
              fprintf (stderr, "return_result: %d\n", return_result (fd, getContentType(cache[cacheIndex].request), cache[cacheIndex].content, cache[cacheIndex].len));
            }
        }
        else
        {
