...
void deleteCache(int idx){
    ...
    else if(cache[idx].len != -1 && cacheStat[idx] != 0)
    {
        printf("-------------------------------------------------------------------------------->IN CACHE\n");
        cache[idx].len = -1;
        cache[idx].request = NULL;
        cache[idx].content = NULL;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < num_dispatcher; i++)
    {
        ...
        threadSet[i] = i;
        err = pthread_create(&dispatchID[i], NULL, dispatch, &threadSet[i]);
	if(err != 0){perror("Thread could not be created: ");}
    }
