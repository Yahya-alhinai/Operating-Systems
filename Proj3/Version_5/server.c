...
int getCacheIndex(char *request){
  /// return the index if the request is present in the cac
  int n = 0;
  for(n; n < 100; n++)
  {
	  if (cache[n].len != 0)
	  {
	  	if(strcmp(cache[n].request, request) == 0)
	  	{
			return n;
	  	}
	  }
...
void deleteCache(int cache_size, int index){
  ...
  if (index == -1)
  {
  	for(n; n < 100; n++)
  	{
    		...
    		{
      			cache[n].len = 0;
    			cache[n].request = NULL;
    			cache[n].content = NULL;
		}
  	}
  }
...
int main(int argc, char **argv) {
  ...
  // Error check on number of arguments
     ...
     printf ("K is: %d\n",k);
     k = getCacheIndex("HELLO");
     printf ("K is: %d\n",k);
     k = getCacheIndex("HELLO2");
     printf ("K is: %d\n",k);
     
     deleteCache(cache_size, 1);
     k = getCacheIndex("HELLO1");
     printf ("K is: %d\n",k);
