...
void build(target_t targets[], int nTarget, int nTargetCount)
{
	for(int j = 0; j < targets[nTarget].DependencyCount; j++) //visit all the dependencies of the target
	{
		...
		if (idx_depend == -1) // The dependency is not a target
		{
			...
			else if(j == targets[nTarget].DependencyCount-1 && compile(targets, nTarget) && parse_into_tokens(targets[nTarget].Command, tokens, " ")) //if it is the last dependency of the target, means that we have checked all the dependency
			{
		...
		else
		{
			...
			if(j == targets[nTarget].DependencyCount-1 && compile(targets, nTarget) && parse_into_tokens(targets[nTarget].Command, tokens, " ")) // compile the command once you check all the Dependencies
			{
				printf("%s\n", targets[nTarget].Command);
				Fexec(tokens, targets[nTarget].TargetName);
			}
