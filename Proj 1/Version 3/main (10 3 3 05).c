...
* login: nguy2625 (login used to submit)
* date: 10/05/2018
* name: Khai Nguyen, Yahya Alhinai (for partner(s))
* id: 5112962, 5181597 */
...
/*-------------------------------------------------------GLOBALE VARIABLES-------------------------------------------*/
int up_to_date = 0;
char *tokens[256]; //This will be used for execvp
...
//This function will check if the targets need to be recompiled
//It uses compare_modification_time function.
//return 1 if targets need to be recompiled, else return 0.
int compile(target_t targets[], int nTarget)
{
	...
	{
		for (int i = 0; i < targets[nTarget].DependencyCount; i++)//check all of the target's dependencies
		{
			if (compare_modification_time(targets[nTarget].TargetName, targets[nTarget].DependencyNames[i]) == 2 )
			{
				return 1; //if the time of those two file are not idential --> compile!
			}
		}
	}
	return 0;//Target file exist, All of the dependencies are checked, don't compile
}
...
void Fexec(char *args[], char *TargetName)
{
    ...
    if(childpid == 0)
		...
		{
			wait(&wstatus);
			//If the parent failed to wait for the child, throw errors and exit
...
{
	char cmd[256];
	//if the target doesn't have any dependency, just run the commands
	if (targets[nTarget].DependencyCount == 0 && targets[nTarget].Status == 0)
	{
		strcpy(cmd, targets[nTarget].Command); //copy the command to "cmd"
		targets[nTarget].Status = 1; //update the status so that it only compile once
		printf("%s\n", targets[nTarget].Command); //Print out the command that about to be executed
		parse_into_tokens(cmd, tokens, " "); //split the command word by word.
		Fexec(tokens, targets[nTarget].TargetName); //Fork a child and execute the command
	}
	for(int j = 0; j < targets[nTarget].DependencyCount; j++) //visit all the dependencies of the target
	{
		strcpy(cmd, targets[nTarget].Command); //copy the command to "cmd"
		//idx_depend is the index of the target, if it is -1, it is not a target
		int idx_depend = find_target(targets[nTarget].DependencyNames[j], targets, nTargetCount);
		if (idx_depend == -1) // if this is true, dependency is not a target
		{
			if(does_file_exist(targets[nTarget].DependencyNames[j]) == -1) //If true, dependency file does not exist, throw error and exit
			{
				printf("make4061: *** No rule to make target '%s', needed by '%s'.  Stop.\n", targets[nTarget].DependencyNames[j], targets[nTarget].TargetName);
				exit(-1);
			}
			//if it is the last dependency of the target, means that we have checked all the dependency
			//check if the target need to be recompiled and split the command word by word so execvp can execute
			else if(j == targets[nTarget].DependencyCount-1 && compile(targets, nTarget) && parse_into_tokens(cmd, tokens, " "))
			{
				printf("%s\n", targets[nTarget].Command); //Print the command that is about to be executed
				Fexec(tokens, targets[nTarget].TargetName); //Fork and execute the command
			}
		}
		else //Dependency is also a target
		{
			build(targets, idx_depend, nTargetCount); // first build the dependencies of the target
			// compile the command once you check all the Dependencies
			if(j == targets[nTarget].DependencyCount-1 && compile(targets, nTarget) && parse_into_tokens(cmd, tokens, " "))
			{
...
void build_Make(target_t targets[], int nTargetCount)
{
	build(targets, 0, nTargetCount);//build the first target, index = 0
	if (up_to_date == 0)
